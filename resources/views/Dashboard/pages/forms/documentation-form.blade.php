<form
    action="{{ isset($documentation) ? route('documentations.update', $documentation->id) : route('documentations.store') }}"
    method="POST"
...
>
    ...
    <input
        type="hidden"
        name="scheduling_id"
        value="{{ old('scheduling_id', $scheduling->id ?? $documentation->scheduling_id ?? '') }}"
    >
    <input
        type="hidden"
        name="nama"
    ...
    <div class="mt-4">
        ...
        <input
            ...
            name="foto_1"
            id="foto1"
            class="block w-full mt-1 border-gray-300 rounded shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
        ...
        @if(isset($documentation) && $documentation->foto_1)
            <img
                src="{{ asset('storage/' . $documentation->foto_1) }}"
                class="w-36 mt-2 rounded"
                alt="Foto 1"
            >
        @endif
    </div>
    ...
    <div class="mt-4 {{ isset($documentation->foto_2) ? '' : 'hidden' }}" id="foto2-container">
        ...
        @if(isset($documentation) && $documentation->foto_2)
            <img
                src="{{ asset('storage/' . $documentation->foto_2) }}"
                class="w-36 mt-2 rounded"
                alt="Foto 2"
            >
        @endif
    ...
    <div class="mt-4">
        ...
        @if(isset($documentation) && $documentation->absen_1)
            <img
                src="{{ asset('storage/' . $documentation->absen_1) }}"
                class="w-36 mt-2 rounded"
                alt="Absensi 1"
            >
        @endif
    ...
    <div class="mt-4 {{ isset($documentation->absen_2) ? '' : 'hidden' }}" id="absen2-container">
        ...
        @if(isset($documentation) && $documentation->absen_2)
            <img
                src="{{ asset('storage/' . $documentation->absen_2) }}"
                class="w-36 mt-2 rounded"
                alt="Absensi 2"
            >
        @endif
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 mt-6">
        <a
            href="{{ url()->previous() }}"
            class="inline-flex items-center px-4 py-2 text-gray-600 bg-white border border-gray-300 rounded-lg shadow-sm hover:bg-gray-50"
        >
        ...
        <button
            type="submit"
            class="inline-flex items-center px-4 py-2 text-white bg-indigo-600 rounded-lg shadow hover:bg-indigo-700 focus:ring-2 focus:ring-indigo-500 focus:ring-offset-1"
        >
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function toggleNextUpload(input, container) {
        ...
        // tampilkan jika sudah ada file saat edit
