<form action="{{ isset($role) ? route('roles.update', $role->id) : route('roles.store') }}" method="POST" class="space-y-6">
    ...
    <div>
        ...
        <input
            ...
            name="name"
            id="name"
            class="block w-full rounded-md border border-gray-300 px-3 py-2 text-gray-900 shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200 @error('name') border-red-500 @enderror"
    ...
    <div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
            ...
            @empty
                <p class="text-gray-400 text-sm">Tidak ada permission tersedia.</p>
            @endforelse
        </div>
    </div>
    ...
    <div class="flex flex-wrap justify-between gap-3">
        <a href="{{ route('roles.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-100 border border-gray-300 text-gray-700 rounded shadow-sm hover:bg-gray-200 transition">
            <i class="fa fa-arrow-left mr-2 text-gray-500"></i>
            <span class="font-semibold">Batal</span>
        </a>

        <button type="submit" class="inline-flex items-center px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded shadow-sm transition">
