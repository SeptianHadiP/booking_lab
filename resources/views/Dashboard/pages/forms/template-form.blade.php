<form action="{{ $formAction ?? route('template.store') }}" method="POST" enctype="multipart/form-data">
    @csrf
    @if(isset($template))
        @method('PUT')
    @endif
    ...
    <div class="row g-4">
        ...
        <div class="col-md-6 text-center">
            ...
            <input type="color" name="font_color" value="{{ old('font_color', $template->font_color ?? '#2D9C63') }}"
                   class="form-control form-control-color mx-auto" style="width: 60px; height: 40px;">
        </div>

        {{-- Upload Background --}}
        <div class="col-md-12">
            <label class="form-label fw-semibold">Upload Background Image</label>
            <input type="file" name="template_file" accept="image/*" class="form-control" {{ isset($template) ? '' : 'required' }}>
            @if(isset($template->file_path))
                <div class="mt-2 text-center">
                    <img src="{{ asset($template->file_path) }}" alt="Preview" style="max-width: 150px; height: auto;">
                </div>
            @endif
        </div>
    </div>
    ...
    <div class="mt-5">
        ...
        <div id="preview-area" class="position-relative" style="width: 1123px; height: 794px; border: 1px solid #ccc; overflow: hidden;">
            <img id="bg-preview" src="{{ isset($template->file_path) ? asset($template->file_path) : '' }}"
                 alt="Preview Background"
                 style="width: 100%; height: 100%; object-fit: contain; position: absolute; top: 0; left: 0; {{ isset($template->file_path) ? '' : 'display: none;' }}">
