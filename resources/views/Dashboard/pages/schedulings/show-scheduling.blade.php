    ...
    <div class="card shadow-sm border-0 rounded-4">
        ...
        <div class="card-body p-4" style="font-size: 16px;">
            ...
            <div class="d-flex justify-content-end gap-2">
                <a href="{{ route('schedulings.index') }}" class="btn btn-light border rounded-pill">
                    <i class="fa fa-arrow-left"></i> Kembali
                </a>
                <a href="{{ route('scheduling.edit', $schedule->id) }}" class="btn btn-warning text-white rounded-pill">
                    <i class="fa fa-pencil"></i> Edit
                </a>
                <form action="{{ route('scheduling.destroy', $schedule->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Yakin ingin menghapus jadwal ini?')">
                    @csrf
                    @method('DELETE')
